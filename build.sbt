...
// scalacommon-test JAR split, containing only test utilities, e.g., AuxSpecs.
lazy val scalaCommonTest = (project in file("."))
  .settings(sharedSettings)
  .settings(
    name := testArtifactName,
    Compile / packageBin / mappings :=
      (Compile / packageBin / mappings).value.filterNot { case (_, path) => isTestCode(path) },
    Compile / packageBin / mappings := {
      val base = (Compile / products).value.head
      val pathFinder = base ** "*"
      pathFinder.get.flatMap { file =>
        val path = IO.relativize(base, file).get
        if (isTestCode(path)) Some(file -> path) else None
      }
    },
  )

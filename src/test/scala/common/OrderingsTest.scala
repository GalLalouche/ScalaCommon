...
class OrderingsTest extends FreeSpec with AuxSpecs {
  "fromSeqCheck" - {
    def seq = Vector("foo", "moo", "bar")
    "Orders by sequence" in {
      Vector("foo", "bar", "foo").sorted(Orderings.fromSeqCheck(seq)) shouldReturn Vector("foo", "foo", "bar")
    }
    "Throw on element not found" in {
      a[NoSuchElementException] shouldBe thrownBy {Vector("foo", "bazz").sorted(Orderings.fromSeqCheck(seq))}
    }
  }
  "fromSeqPartial" - {
    def seq = Vector("foo", "moo", "bar")

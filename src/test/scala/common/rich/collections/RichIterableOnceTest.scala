...
class RichIterableOnceTest extends FreeSpec with AuxSpecs {
  ...
  "unorderedPairs" - {
    "has size of n choose 2" in {
      Vector.fill(10)("bla").unorderedPairs.iterator.size shouldReturn 45
    }

    "returns all unique pairs" in {
      Vector(1, 2, 3).unorderedPairs.iterator.toSet shouldReturn Set((1, 2), (1, 3), (2, 3))
    }
  }

  "joinWhere" - {
    "returns an empty iterable if one of the iterables is empty" in {
      Vector(1, 2, 3).join(Vector[String]()).where((_, _) => true).iterator shouldBe empty
      Vector[Int]().join(Vector[String]("1", "2", "3")).where((_, _) => true).iterator shouldBe empty
    }

    "returns a cartesian product if the 'where' function is true" in {
      Vector(1, 2, 3).join(Vector(4, 5, 6)).where((_, _) => true).iterator.toSet.size shouldBe 9
    }

    "filters by the where function" in {
      Vector(1, 2).join(Vector(4, 5)).where((e, f) => e * f % 2 == 0).iterator.toSet shouldBe Set((2, 4), (1, 4), (2, 5))
    }
  }
  ...
  "joinBy" - {
    "joins two items whose extractor returns the same value" in {
      Vector(Person("gal", 1), Person("noam", 2)).join(Vector(Grade(1, 100), Grade(2, 0))).by(_.id, _.id).iterator.toVector shouldBe
    ...
    "creates a new object if such a builder is provided " in {
      Vector(Person("gal", 1), Person("noam", 2))
          ...
          .iterator.toSet shouldBe Set(PersonWithGrade("gal", 1, 100), PersonWithGrade("noam", 2, 0))
    }
  }

  "foreachWithInBetween performs the selectedActionInBetween each foreach" in {
    val actual = mutable.ArrayBuffer[String]()
  ...
  "selectRepresentative selects a representative from each class" in {
    (1 to 100).selectRepresentative(_ % 5).iterator.map(_ % 5).toSet.size shouldBe 5
  }
  ...
  "unorderedPairs" - {
    "produces no repeats" in {
      Vector(1, 2).unorderedPairs.iterator.toSet.size shouldReturn 1
    }

    "produces pairs" in {
      Vector(1, 2, 3).unorderedPairs.iterator.toSet shouldReturn Set((1, 2), (2, 3), (1, 3))
    }
  }
  ...
  "toMultimap" - {
    "creates a multimap" in {
      Vector("one", "two", "three").toMultiMap(_.length) shouldReturn Map(3 -> Seq("one", "two"), 5 -> Seq("three"))
    }
    "extracts the value when passed" in {
      Vector("one", "two", "three").toMultiMap(_.length, _.toUpperCase) shouldReturn Map(3 -> Seq("ONE", "TWO"), 5 -> Seq("THREE"))
    }
  }
  ...
  "filterAndSortBy" - {
    ...
    "filters and sort by" in {
      Seq("one", "four", "three").filterAndSortBy(_.length, Vector(5, 3)) shouldReturn Vector("three", "one")
    }
  }

  "product works on iterators" in {
    val $ = Iterator(1, 2, 3) * Iterator(4, 5, 6)
    $.iterator.toVector shouldReturn Vector(1 -> 4, 1 -> 5, 1 -> 6, 2 -> 4, 2 -> 5, 2 -> 6, 3 -> 4, 3 -> 5, 3 -> 6)
  }

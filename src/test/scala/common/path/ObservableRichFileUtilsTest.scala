...
class ObservableRichFileUtilsTest
  ...
  "moveContents" - {
    "empty directory" in {
      ObservablePathUtils.moveContents(srcDir, dstDir, observer)
    ...
    "directory with 3 files" in {
      ...
      IOFile(dstDir / "file1.txt").readAll shouldReturn "content1"
      IOFile(dstDir / "file2.txt").readAll shouldReturn "content2"
      IOFile(dstDir / "file3.txt").readAll shouldReturn "content3"
      steps
    ...
    "nested directory structure" in {
      ...
      IOFile(dstDir / "rootFile.txt").readAll shouldReturn "rootContent"
      ...
      IOFile(movedSubdir1 / "file1.txt").readAll shouldReturn "content1"
      ...
      IOFile(movedSubdir2 / "file2.txt").readAll shouldReturn "content2"
      IOFile(movedSubdir2 / "file3.txt").readAll shouldReturn "content3"
      val movedDir2 = IODirectory(dstDir / "dir2")
      movedDir2.files
        .map(_.getName)
        .toVector
        .shouldContainExactly("file4.txt", "file5.txt", "file6.txt")
      IOFile(movedDir2 / "file4.txt").readAll shouldReturn "content4"
      IOFile(movedDir2 / "file5.txt").readAll shouldReturn "content5"
      IOFile(movedDir2 / "file6.txt").readAll shouldReturn "content6"
      steps
        .map(_.current.getName)

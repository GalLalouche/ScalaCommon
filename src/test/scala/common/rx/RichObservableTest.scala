...
import common.rx.RichObservable.{richObservable, Unsubscribable}
...
class RichObservableTest extends AsyncFreeSpec with AsyncAuxSpecs {
  ...
  "richObservable" - {
    ...
    "doOnNextAsync" - {
      ...
      "All valid" in {
        val sub = createSubject
        val queue = ArrayBuffer[Int]()
        val future = sub.doOnNextAsync(e => Future(queue += e)).toFuture[Vector]
        sub.onNext(1)
        sub.onNext(2)
        sub.onNext(3)
        sub.onCompleted()
        future.map(res =>
          (res shouldReturn Vector(1, 2, 3)) &&
            (queue.toVector shouldReturn Vector(1, 2, 3)),
        )
      }
      "Should not proceed on synced error, and return the correct error" in {
        runAndVerifyError { (i, buf) =>
          i match {
            case e if e < 3 => Future(buf += e)
            case _ => throw e
          }
        }
      }
      ...
      def runAndVerifyError(f: (Int, ArrayBuffer[Int]) => Future[_]) = {
        val sub = createSubject
        val queue = ArrayBuffer[Int]()
        ...
        for {
          ...
          res <- failure && (queue.toVector shouldReturn Vector(1, 2))
        } yield res
      }
    }
    ...
    "groupByBuffer" - {
      "empty returns an empty observable" in {
        Observable.just[Int]().groupByBuffer(_ => ???).toFuture[Vector].map(_ shouldBe empty)
      }
      "Single element returns the single element" in {
        Observable.just(1).groupByBuffer(_.toString).toFuture[Vector] shouldEventuallyReturn Vector(
          "1" -> Vector(1),
        )
      }
      "No repeats returns singletons" in {
        ...
        Observable
          .just(1, 2, 3)
          .groupByBuffer(_.toString)
          .toFuture[Vector] shouldEventuallyReturn expected
      }
      "Groups by key" in {
      ...
      "Lazy (blocking)" in {
        ...
        RichObservable
          ...
          .take(5)
          .toFuture[Vector] shouldEventuallyReturn 0
          .to(4)
          .toVector
          .fproduct(e => Vector(2 * e, 2 * e + 1))
      }
      "Lazy (infinite)" in {
        RichObservable
          .iterate(0)(_ + 1)
    ...
    "Object methods" - {
      ...
      "concat" - {
        "empty" in {
          RichObservable
            ...
            .toFuture[Vector] shouldEventuallyReturn Vector.empty
        }
        "simple" in {
          RichObservable
            .concat(Vector(Observable.just(1), Observable.just(2)))
            .toFuture[Vector] shouldEventuallyReturn Vector(1, 2)
        }
        "reusable" in {
        ...
        "large list" in {
          RichObservable
            .concat(1.to(10000).map(Observable.just(_)))
            .toFuture[Vector] shouldEventuallyReturn 1.to(10000).toVector
        }
        "unsubscribes" in {
      ...
      "continually" in {
        RichObservable.continually("foo").take(10).toFuture[Vector] shouldEventuallyReturn Vector
          .fill(10)("foo")
      }
      "iterate" in {
        RichObservable.iterate(0)(_ + 1).take(10).toFuture[Vector] shouldEventuallyReturn 0
          .to(9)
          .toVector
      }
    }

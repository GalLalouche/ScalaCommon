...
abstract class UnionFindTest extends AnyFreeSpec with AuxSpecs {
  ...
  "union" - {
    "after union, both items are in the same set" in {
      ...
      deepSets(unioned.sets) shouldEqual Set(
        Set("foo", "bar"),
        Set("bazz"),
        Set("quux"),
        Set("moo"),
  ...
  "representative" - {
    ...
    "representative is consistent" in {
      val uf =
        createUnionFind(Vector(1, 2, 3, 4, 5, 6)).union(1, 2).union(2, 3).union(4, 5).union(4, 6)
      assertAll(
        Vector(1, 2, 3).map(e => uf.getRepresentative(e) shouldReturn uf.getRepresentative(3)),
      )
      assertAll(
        Vector(4, 5, 6).map(e => uf.getRepresentative(e) shouldReturn uf.getRepresentative(4)),
      )
    }
  }
  "set" - {

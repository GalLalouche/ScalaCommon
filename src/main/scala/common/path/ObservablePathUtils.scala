...
/** Like [[PathUtils]], but with observable progress reporting. */
// PlainSeqInstances extension needed for 2.12 source backward-compatibility.
object ObservablePathUtils extends PlainSeqInstances {
  ...
  /** See the equivalent method in [[PathUtils]]. */
  ...
  def moveContents(
    ...
    def aux(src: IODirectory, dst: IODirectory): State[InternalProgress, Unit] = {
      def moveFile(f: File): State[InternalProgress, Unit] = {
        PathUtils.move(IOFile.unsafe(f), dst)
        RichState

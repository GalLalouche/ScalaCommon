...
/** Scalaz's [[scalaz.std.scalaFuture.futureInstance]] has an async point implementation, which is silly. */
trait BetterFutureInstances {
  ...
  implicit def betterFutureInstances(implicit ec: ExecutionContext): MonadError[Future, Throwable] =
    new MonadError[Future, Throwable] {
      ...
      override def handleError[A](fa: Future[A])(f: Throwable => Future[A]) = fa.recoverWith {case e => f(e)}

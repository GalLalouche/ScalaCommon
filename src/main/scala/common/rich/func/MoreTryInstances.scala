...
trait MoreTryInstances {
  implicit object TryInstances extends TryInstances with MonadPlus[Try] with MonadError[Try, Throwable] {
    override def bind[A, B](fa: Try[A])(f: A => Try[B]): Try[B] = fa flatMap f
    override def empty[A]: Try[A] = Failure(new NoSuchElementException)
    override def plus[A](a: Try[A], b: => Try[A]): Try[A] = a orElse b

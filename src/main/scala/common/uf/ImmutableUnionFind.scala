 ...
 * shortening (which actually ensures O(log*(n)) performance) can only be done on unions, but not on
 * sameSet checks. In other words, this could mean that the actual performance would be O(n) in the
 * worst case for some specific union operations. This could in theory be implemented using a State
 * monad, but we aren't in Haskell land anymore. See [[MutableUnionFind]] for the by-the-book
 * implementation and performance.
 */
class ImmutableUnionFind[A] private (
    parents: Vector[Int],
    protected override val index: Map[A, Int],
    override val numberOfSets: Int,
) extends AbstractUnionFind[A] {
  override type UF = ImmutableUnionFind[A]
  protected override def getSet(a: A): Int = getPathToParent(a).head

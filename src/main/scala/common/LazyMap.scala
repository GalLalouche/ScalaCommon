...
/**
 * A LazyMap is basically a memoized function. That is, if the value matching the key does not
 * exist, the function will be applied to create it. This class is immutable.
 */
class LazyMap[K, V] private (f: K => V, currentMap: Map[K, V]) {
  /**
   * Gets the value for the key, and a (possibly-updated) map with an association with the key and
   * value. If the key did not exist in the map, the updated map will be returned. Otherwise, the
